...
/* print_welcome_banner: 시작 시 ASCII art 환영 화면 (세인 작업, PR #31).
 *
 * Hello Kitty (47줄) + WELCOME 큰 글씨 + SQL PARSER 큰 글씨 를 동시에 출력.
 * 외부 의존성 0 (모두 정적 문자열). 발표 시연 임팩트용.
 *
 * 출력은 stdout 으로만 가므로 단위 테스트에는 영향 없음 (테스트는 freopen
 * 으로 stdout 을 캡쳐하지만 main() 자체는 거의 호출 안 함).
 */
