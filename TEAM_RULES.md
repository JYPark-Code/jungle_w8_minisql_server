...
- ❌ CI 실패 상태로 merge
- ❌ `include/*.h` PM 승인 없이 수정 (단, **새 모듈 헤더** (예: Round 2 의 `src/dict_cache.h` private 헤더, `include/trie.h`) 는 해당 담당자가 생성. 기존 5 개 헤더 수정은 여전히 PM 승인)
...
|---|---|---|---|---|
| 지용 (Lead, TP) | `feature/dynamic-threadpool` | `src/threadpool.c`, `include/threadpool.h` (API 추가), `src/server.c` (graceful shutdown 연계), `tests/test_threadpool.c` 보강 | `src/main.c` (signal handler 보강) | `src/dict_cache.c`, `src/dict_cache.h`, `src/trie.c`, `src/engine.c`, `src/router.c`, `web/` |
| 동현 (Dict Cache) | `feature/lru-cache` | `src/dict_cache.c` (신규), `src/dict_cache.h` (신규, private 헤더), `src/router.c` 의 Zone R-DICT-CACHE / R-STATS / R-INIT, `tests/test_dict_cache.c` (신규) | — (engine.c 는 **건드리지 않음**, server.c 는 금지) | `src/server.c`, `src/engine.c`, `src/threadpool.c`, `src/trie.c`, `web/` |
| 용 (Trie) | `feature/trie-prefix` | `src/trie.c` (신규), `include/trie.h` (신규), `src/engine.c` 의 Zone T1 + IT (prefix dispatcher + trie init/teardown), `src/router.c` 의 Zone R-AUTO / R-ADMIN-INSERT, `tests/test_trie.c` (신규) | (없음) | `src/server.c`, `src/threadpool.c`, `src/dict_cache.c`, `src/dict_cache.h`, `web/` |
...
- **거절 / 확정 예시 (Round 2 회의 결정)**:
  ...
  - ❌ 동현 가 `src/engine.c` 에 `dict_cache_get` / `dict_cache_put` 을
    직접 삽입하는 것
  ...
  - ✅ 동현 가 `src/router.c` 의 `/api/stats` 핸들러에 `cache_hits/misses`
    응답 필드를 추가하는 것 → 허용 (Zone R-STATS). endpoint 1 개 응답
    확장이라 용 의 신규 endpoint 추가와 conflict 낮음 (응답 JSON 키는
    사용자 facing 이라 `cache_*` 짧은 형태 유지)
