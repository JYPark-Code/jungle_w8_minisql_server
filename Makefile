...
ROUTER_TEST_DEPS     = $(SRC_DIR)/router.c $(SRC_DIR)/dict_cache.c \
                       $(SRC_DIR)/engine.c $(SRC_DIR)/engine_lock.c \
                       $(SRC_DIR)/trie.c $(W7_SRCS)
...
.PHONY: all clean test tsan valgrind bench loadtest repl \
        test_storage_all \
        test_bptree test_benchmark test_index_registry test_engine_lock test_threadpool test_trie test_dict_cache test_router
...
# 회귀 테스트 (W7 + 추후 W8)
# ---------------------------------------------------------------------------
test: $(TEST_PARSER_EXEC) test_storage_all test_bptree test_benchmark test_index_registry test_engine_lock test_threadpool test_trie test_dict_cache test_router
	./$(TEST_PARSER_EXEC)
...
test_router: $(TEST_DIR)/test_router.c $(ROUTER_TEST_DEPS)
	$(CC) $(CFLAGS) -o $@ $^ $(LDFLAGS)
	./$@
...
# ---------------------------------------------------------------------------
valgrind: $(TEST_PARSER_EXEC) $(STORAGE_TEST_TARGETS) test_bptree test_benchmark test_index_registry test_engine_lock test_threadpool test_trie test_dict_cache test_router
	...
	@echo "=== valgrind: router ==="
	valgrind --leak-check=full --error-exitcode=1 --quiet ./test_router
