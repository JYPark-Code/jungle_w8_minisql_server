...
1. **동시 조회 (`/api/dict?word=apple`).** 다수 사용자가 동시에 영어 단어
   로 한글 뜻 조회. router 의 dict cache 가 반복 조회를 흡수.
...
| Method | Path | 설명 |
|---|---|---|
| `GET` | `/api/dict?word=<english>` | 영한 조회 (primary). dict cache 경유. cache miss 시 engine 호출 |
| `GET` | `/api/dict?korean=<한글>` | 역방향 조회 (option B). 인덱스 없음, 선형 스캔 경로 (느림) |
